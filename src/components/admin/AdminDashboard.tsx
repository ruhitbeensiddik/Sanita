...
export function AdminDashboard() {
  ...
  const adminTabs: { id: AdminTab; label: string; icon: any }[] = isSuperAdmin ? [
    ...
    { id: 'payment-requests', label: 'Payments', icon: CreditCard },
  ] : [
    { id: 'users', label: 'Users', icon: Users },
  ]
  ...
  return (
    <div className="space-y-6 pb-20">
      ...
      {/* Admin Sub-Tabs */}
      {isSuperAdmin && (
        <div className="flex flex-wrap gap-2 bg-card p-3 rounded-xl border border-border shadow-sm">
          {adminTabs.map(tab => {
            const Icon = tab.icon
        ...
        <>
              ...
              <CardHeader>
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <CardDescription>{pendingUsers.length} user{pendingUsers.length !== 1 ? 's' : ''} awaiting your approval.</CardDescription>
                  </div>
                </div>
              </CardHeader>
              <CardContent>
                <div className="overflow-x-auto w-full">
                  <table className="w-full text-sm text-left">
                    <thead className="text-xs text-muted-foreground uppercase bg-amber-500/5 border-b border-amber-500/20">
                      <tr><th className="px-4 py-3">Email</th><th className="px-4 py-3">Registered</th><th className="px-4 py-3 text-right">Actions</th></tr>
                    </thead>
                        ...
                        <tr key={user.id} className="hover:bg-amber-500/5">
                          <td className="px-4 py-4 font-medium text-foreground">
                            <div className="flex items-center gap-2">{user.email}
                              <span className="inline-flex items-center gap-1 px-1.5 py-0.5 rounded text-[10px] font-bold bg-amber-500/10 text-amber-500 border border-amber-500/20"><Clock className="h-2.5 w-2.5" /> PENDING</span>
                            </div>
                          </td>
          ...
          <Card>
            <CardHeader>
              ...
              <CardDescription>{isSuperAdmin ? 'Full user management — inspect, promote, demote, or delete users.' : 'View users, promote to admin, or remove users.'}</CardDescription>
            </CardHeader>
            <CardContent>
              <div className="overflow-x-auto w-full">
                <table className="w-full text-sm text-left">
                    ...
                    {users.map(user => {
                      ...
                      return (
                        <tr key={user.id} className="hover:bg-muted/30">
                          ...
                          <td className="px-4 py-4">{accounts.filter(a => a.userId === user.id).length}</td>
                          <td className="px-4 py-4 text-right">
                              ...
                              {!isSelf && (
                                <>
                                  ...
                                  {!isProtected && (
                                    <>
                                      {isSuperAdmin && !isTargetSuperAdmin && (
                                        <Button size="sm" variant="outline" onClick={() => handleDelete(user.id)} className="h-8 text-xs border-destructive/30 text-destructive hover:bg-destructive/10 leading-none"><Trash2 className="h-3 w-3 mr-1"/> Delete</Button>
                                      )}
                                  ...
                                  {isProtected && !isSelf && <span className="text-xs text-muted-foreground italic">Protected</span>}
                                </>
                              )}
                            </div>
                          </td>
      ...
      <Suspense fallback={<div className="p-6 text-muted-foreground text-center">Loading...</div>}>
        ...
        {activeAdminTab === 'payment-requests' && <AdminPaymentRequests />}
      </Suspense>
    </div>
  )
}
